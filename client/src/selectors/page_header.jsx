...
const allPageHeadersAsArray = createSelector([allPageHeaders], (pageHeaders) => pageHeaders.valueSeq())
...
export const selectPageHeaderKind = createSelector([selectPathname], (pathname) => {
  if (pathname === '/') {
    ...
    pathname === '/auth/reset-my-password' ||
...
export const selectPageHeaderSlug = createSelector([selectPathname], (pathname) => {
  ...
  if (/\/discover\/(all|subscribed|featured|trending|recent|shop).*/i.test(pathname)) {
    return null
  }
...
export const selectPageHeaders = createSelector(
  ...
  (pageHeaders, slug, kind) =>
    pageHeaders.filter((header) => header.get('kind') === kind && (!slug || slug === header.get('slug')))
)
