...
import { selectParamsUsername } from './params.jsx'
...
export function selectDiscoverStreamKind(state, { params: { stream, kind } }) {
  ...
  if (stream === 'recent' || stream === 'trending' || stream === 'shop') {
    return stream
  }
  if (!kind) {
    return 'featured'
...
// Memoized selectors
export const selectViewNameFromRoute = createSelector([selectPathname, selectParamsUsername], (pathname, username) => {
  if (pathname === '/') {
    return 'editorial'
  }
...
export const selectIsAuthenticationView = createSelector(
  ...
  (viewName) => viewName === 'authentication'
)

export const selectIsOnboardingView = createSelector([selectViewNameFromRoute], (viewName) => viewName === 'onboarding')

export const selectIsPostDetail = createSelector([selectViewNameFromRoute], (viewName) => viewName === 'postDetail')

export const selectIsGlobalRoot = createSelector([selectPathname], (pathname) =>
  /^\/(?:discover(\/featured|\/recommended|\/trending|\/recent|\/shop)?)?$/.test(pathname)
)

export const selectIsSubscribedRoot = createSelector([selectPathname], (pathname) =>
  /^\/(?:discover\/subscribed(\/trending|\/shop|\/recent)?)?$/.test(pathname)
)

export const selectShowCategoryHeader = createSelector(
  [selectIsGlobalRoot, selectIsSubscribedRoot],
  (isGlobalRoot, isSubscribedRoot) => isGlobalRoot || isSubscribedRoot
)
